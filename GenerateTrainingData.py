...
for param in param_list:
    ...
    p1 = [param["instance_size"]]*mp.cpu_count()
    p2 = [param["sample_size"]]*mp.cpu_count()
    p3 = [param["seed"]]*mp.cpu_count()
    p3 = [item+i for i,item in enumerate(p3)]
    print(p1,p2,p3)

    cores = mp.cpu_count()
    pool = Pool(cores)
    data = np.vstack(pool.map(NR, p1, p2, p3))
